...
describe GildedRose do
  ...
  describe "#update_quality" do
    it "does not change the name" do
      ...
      GildedRose.new([item]).update_generic_item_quality(item)
    ...
    context 'when item is Aged Brie' do
      context 'before sell_in' do
        it 'raises quality by one after a day' do
          item = Item.new(GildedRose::AGED_BRIE, 1, 0)
          GildedRose.new([item]).update_aged_brie(item)
          expect(item.quality).to eq(1)
        end

        it 'never raises quality beyond 50' do
          item = Item.new(GildedRose::AGED_BRIE, 1, 50)
          GildedRose.new([item]).update_aged_brie(item)
      ...
      context 'after sell_in' do
        it 'raises quality by two after a day' do
          item = Item.new(GildedRose::AGED_BRIE, 0, 0)
          GildedRose.new([item]).update_aged_brie(item)
          expect(item.quality).to eq(2)
        end

        it 'only raises quality to 50 when quality is at 49' do
          item = Item.new(GildedRose::AGED_BRIE, 0, 49)
          GildedRose.new([item]).update_aged_brie(item)
          expect(item.quality).to eq(50)
        end
      end
    ...
    context 'when item is Backstage pass for a Metallica concert' do
      context 'when sell_in is greater than 10' do
        it 'increases quality by one after each day' do
          item = Item.new(GildedRose::BACKSTAGE_PASS, 15, 0)
          GildedRose.new([item]).update_backstage_pass(item)
          expect(item.quality).to eq(1)
        end

        it 'never increases quality beyond 50' do
          item = Item.new(GildedRose::BACKSTAGE_PASS, 15, 50)
          GildedRose.new([item]).update_backstage_pass(item)
          expect(item.quality).to eq(50)
        end
      end
      ...
      context 'when sell_in is less than or equal to 10 but more than 5' do
        it 'increases quality by two after each day' do
          item = Item.new(GildedRose::BACKSTAGE_PASS, 10, 0)
          GildedRose.new([item]).update_backstage_pass(item)
          expect(item.quality).to eq(2)
        end

        it 'only raises quality to 50 when quality is at 49' do
          item = Item.new(GildedRose::BACKSTAGE_PASS, 10, 49)
          GildedRose.new([item]).update_backstage_pass(item)
          expect(item.quality).to eq(50)
        end
      end
      ...
      context 'when sell_in is less than or equal to 5 but more than 0' do
        it 'increases quality by three after each day' do
          item = Item.new(GildedRose::BACKSTAGE_PASS, 5, 0)
          GildedRose.new([item]).update_backstage_pass(item)
          expect(item.quality).to eq(3)
        end

        it 'only raises quality to 50 when quality is at 48 or more' do
          item = Item.new(GildedRose::BACKSTAGE_PASS, 5, 48)
          GildedRose.new([item]).update_backstage_pass(item)
          expect(item.quality).to eq(50)
        end
      end
      ...
      context 'after sell_in' do
        it 'sets quality to 0' do
          item = Item.new(GildedRose::BACKSTAGE_PASS, 0, 50)
          GildedRose.new([item]).update_backstage_pass(item)
          expect(item.quality).to eq(0)
        end

        it 'does not change quality' do
          item = Item.new(GildedRose::BACKSTAGE_PASS, 0, 0)
          GildedRose.new([item]).update_backstage_pass(item)
          expect(item.quality).to eq(0)
        end
      end
